...
import com.excilys.ebi.gatling.charts.config.ChartsFiles.{ jsStatsFile, tsvStatsFile }
...
class StatsReportGenerator(runOn: String, dataReader: DataReader, componentLibrary: ComponentLibrary) {
	...
	def generate: Map[String, RequestStatistics] = {
		val criteria: List[(String, Option[String])] = ("global_information", None) :: dataReader.requestNames.map(name => (name, Some(name))).toList
		...
		val stats = criteria.map { criterion =>
			val (name, requestName) = criterion
