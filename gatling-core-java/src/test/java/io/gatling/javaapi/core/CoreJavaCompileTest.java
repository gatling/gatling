  ...
  private ScenarioBuilder scenario =
      scenario("scenario")
          ...
          .exec(
              ...
              feed(separatedValues("foo", '|', '"')),
              feed(csv("foo").headers("foo", "bar")),
              feed(ssv("foo").headers("foo")),
              feed(tsv("foo").headers("foo", "bar")),
              feed(separatedValues("foo", '|').headers("foo", "bar")),
              feed(csv("foo").random().unzip().shard().headers("foo", "bar")),
              feed(csv("foo").headers("foo", "bar").random().unzip().shard()),
              feed(jsonFile("foo")),
              feed(jsonlFile("foo")),
              feed(jsonUrl("foo")),
              feed(
              ...
              pause(
                  session -> Duration.ofMillis(100),
                  session -> Duration.ofMillis(200),
                  constantPauses),
