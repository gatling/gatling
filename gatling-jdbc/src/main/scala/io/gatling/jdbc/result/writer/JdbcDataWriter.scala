...
class JdbcDataWriter extends DataWriter with Logging {
	...
	override def onRequestMessage(request: RequestMessage) {
		...
		requestInsert.setLong(5, firstByteSent)
		requestInsert.setLong(6, lastByteSent)
		requestInsert.setLong(7, firstByteReceived)
		requestInsert.setLong(8, lastByteReceived)
