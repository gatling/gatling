...
class Gatling extends Logging {
	...
	def start = {
		val (outputDirectoryName, simulation) = GatlingFiles.reportsOnlyDirectory.map((_, None))
			.getOrElse {
				...
				(new Runner(selection, simulation).run, Some(simulation))
			}

		val dataReader = DataReader.newInstance(outputDirectoryName)

		val isSuccessful = checkSimulation(simulation, dataReader)

		if (!configuration.charting.noReports) generateReports(outputDirectoryName, dataReader)

		if (isSuccessful) Gatling.SUCCESS else Gatling.SIMULATION_CHECK_FAILED
	}
	...
	private def checkSimulation(simulation: Option[Simulation], dataReader: DataReader) = {

		simulation match {
			case Some(simulation) => {
				val successful = Assertion.assertThat(simulation.isSuccessful, dataReader)
