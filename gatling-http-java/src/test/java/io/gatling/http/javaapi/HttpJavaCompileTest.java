...
public class HttpJavaCompileTest extends Simulation {
  ...
  HttpProtocolBuilder http = http()
    ...
    .perUserKeyManagerFactory(session -> (KeyManagerFactory) null)
    .disableAutoReferer()
    .disableAutoOrigin()
    .disableCaching()
    .header("name", "value")
    ...
    .signWithOAuth1(session -> "consumerKey", session -> "clientSharedSecret", session -> "token", session -> "tokenSecret")
    .enableHttp2()
    ...
    .check(
      ...
      regex("pattern").capture2().is(new RegexGroups.Tuple2("foo", "bar")),
  ...
  public HttpJavaCompileTest() {
    ScenarioBuilder scn = scenario("scenario")
      .exec(
        http("name")
          ...
          .queryParamSeq(Collections.singletonList(new AbstractMap.SimpleImmutableEntry<>("foo", "bar")))
          .queryParamSeq(session -> Collections.singletonList(new AbstractMap.SimpleImmutableEntry<>("foo", "bar")))
          ...
          .formParamSeq(Collections.singletonList(new AbstractMap.SimpleImmutableEntry<>("foo", "bar")))
          .formParamSeq(session -> Collections.singletonList(new AbstractMap.SimpleImmutableEntry<>("foo", "bar")))
          ...
          .bodyParts(
            RawFileBodyPart("name1", "path1"),
            RawFileBodyPart("name2", "path2")
          )
      ...
      // processRequestBody
      .exec(http("Request").post("/things").body(StringBody("FOO${BAR}BAZ")).processRequestBody(Function.identity()))
    ...
    setUp(
      scn.injectOpen(atOnceUsers(1)).protocols(http)
    ).protocols(http);
  }
}
