...
object ScalaWsFrameCheckTextConditions {
  ...
  def untyped(context: io.gatling.http.check.ws.WsFrameCheck.Text, condition: JavaExpression[jl.Boolean]): Untyped =
    new Untyped(context, javaBooleanFunctionToExpression(condition))

  final class Untyped(context: io.gatling.http.check.ws.WsFrameCheck.Text, condition: Expression[Boolean]) {
    def then_(checkBuilders: ju.List[CheckBuilder]): WsFrameCheck.Text =
      new WsFrameCheck.Text(context.checkIf(condition)(WsChecks.toScalaTextChecks(checkBuilders): _*))
  }

  def typed(context: io.gatling.http.check.ws.WsFrameCheck.Text, condition: juf.BiFunction[String, Session, jl.Boolean]): Typed =
    new Typed(context, (u, session) => safely()(condition.apply(u, new Session(session)).booleanValue.success))

  final class Typed(context: io.gatling.http.check.ws.WsFrameCheck.Text, condition: (String, ScalaSession) => Validation[Boolean]) {
    def then_(checkBuilders: ju.List[CheckBuilder]): WsFrameCheck.Text =
      new WsFrameCheck.Text(context.checkIf(condition)(WsChecks.toScalaTextChecks(checkBuilders): _*))
  }
}
