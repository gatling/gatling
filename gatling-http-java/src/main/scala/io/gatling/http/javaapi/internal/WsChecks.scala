...
object WsChecks {
  ...
  private def toScalaTextCheck(javaCheck: io.gatling.core.javaapi.CheckBuilder): WsCheck.Text = {
  ...
  def toScalaTextChecks(javaChecks: ju.List[io.gatling.core.javaapi.CheckBuilder]): Seq[WsCheck.Text] =
    javaChecks.asScala.map(toScalaTextCheck).toSeq

  private def toScalaBinaryCheck(javaCheck: io.gatling.core.javaapi.CheckBuilder): WsCheck.Binary = {
    val scalaCheck = javaCheck.asScala
    javaCheck.`type` match {
  ...
  def toScalaBinaryChecks(javaChecks: ju.List[io.gatling.core.javaapi.CheckBuilder]): Seq[WsCheck.Binary] =
