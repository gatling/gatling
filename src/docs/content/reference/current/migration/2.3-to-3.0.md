...
* WebSocket and SSE support were completely re-written, see [new API](||< ref "docs/http-ws" >||).
* `ntmlAuth` is dropped until we can support NTLM properly, see [gatling#3525](https://github.com/gatling/gatling/issues/3525)
* The way to compute HTTP request signatures was revamped, see [new API](||< ref "docs/http-request-signature" >||)
