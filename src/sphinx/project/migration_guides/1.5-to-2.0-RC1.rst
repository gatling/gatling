...
Gatling is now published on Maven Central
-----------------------------------------

Gatling releases are now published on Maven Central, and snapshots are deployed on each successful build by Travis CI to Sonatype's snapshot repository.
If you were using Gatling with its Maven plugin, you no longer need to add Excilys' repository in your ``pom.xml``.
...
* Update your build scripts to the change the groupId of Gatling's dependencies from `com.excilys.ebi.gatling` to `io.gatling`
...
Injection *steps* are now configured using the ``inject`` method available on your configured scenarion
...
For more information on the new Injection DSL, please consult :ref:`Injection reference documentation <simulation-inject>`.
...
``foreach`` the first parameter is now an ``Expression`` (e.g. could be Gatling EL string), not the name of the attribute to loop over.

For example, if you have a list stored in the user's session with under the name ``list``::
...
HTTP protocol boostrapper has been renamed from ``httpProtocol`` to ``http``.
...
More for information on ``extraInfoExtractor``, please refer to its :ref:`documentation <http-protocol-extractor>`.
...
Until now, when setting up the Recorder, it was needed to setup different ports for the Recorder's *local proxy*, one for HTTP, another HTTPS.
