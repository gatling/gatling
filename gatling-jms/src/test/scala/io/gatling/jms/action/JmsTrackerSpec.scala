...
class JmsTrackerSpec extends ActorSpec with CoreDsl with JmsDsl with MockMessage with EmptySession {
  ...
  "JmsRequestTrackerActor" should "pass to next to next actor when matching message is received" in {
    ...
    tracker ! JmsTracker.Command.MessageSent("1", 15, 0, Nil, Nil, emptySession, new ActorDelegatingAction("next", nextActor), "success")
  ...
  it should "pass KO to next actor when check fails" in {
    ...
    tracker ! JmsTracker.Command.MessageSent("1", 15, 0, List(failedCheck), Nil, emptySession, new ActorDelegatingAction("next", nextActor), "failure")
    tracker ! JmsTracker.Command.MessageReceived("1", 30, textMessage("test"))
  ...
  it should "pass updated session to next actor if modified by checks" in {
    ...
    tracker ! JmsTracker.Command.MessageSent("1", 15, 0, List(check), Nil, emptySession, new ActorDelegatingAction("next", nextActor), "updated")
  ...
  it should "pass information to session about response time in case group are used" in {
    ...
    tracker ! JmsTracker.Command.MessageSent("1", 15, 0, Nil, Nil, groupSession, new ActorDelegatingAction("next", nextActor), "logGroupResponse")
    ...
    val failedCheck = simpleCheck(_ => false)
    tracker ! JmsTracker.Command.MessageSent("2", 25, 0, List(failedCheck), Nil, newSession, new ActorDelegatingAction("next", nextActor), "logGroupResponse")
