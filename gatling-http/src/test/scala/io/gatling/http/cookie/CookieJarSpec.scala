...
class CookieJarSpec extends BaseSpec {
  ...
  it should "return multiple cookies even if they have the same name" in {
    val cookieStore = CookieJar(Uri.create("http://foo.com"), List(decode("JSESSIONID=FOO; Domain=.foo.com")))
    val newCookieStore = cookieStore.add(Uri.create("http://sub.foo.com"), List(decode("JSESSIONID=BAR; Domain=sub.foo.com")))
