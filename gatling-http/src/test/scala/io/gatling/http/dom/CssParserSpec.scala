...
class CssParserSpec extends Specification {
	...
	"parsing CSS" should {
		...
		"work with a simple CSS" in {
			...
			rulesUri(css) must beEqualTo(Seq("body" -> "http://akka.io/backgrounds/blizzard.png"))

			val css2 = """body { 
										background-image: url('backgrounds/blizzard.png'); 
								 }"""
			rulesUri(css2) must beEqualTo(Seq("body" -> "http://akka.io/backgrounds/blizzard.png"))
		}

		"work with a all kind of selectors CSS" in {
			val css = """.foo { background-image: url('backgrounds/blizzard.png');}"""
			rulesUri(css) must beEqualTo(Seq(".foo" -> "http://akka.io/backgrounds/blizzard.png"))

			val css2 = """#foo { background-image: url('backgrounds/blizzard.png');}"""
			rulesUri(css2) must beEqualTo(Seq("#foo" -> "http://akka.io/backgrounds/blizzard.png"))

			val css3 = """.foo .bar { background-image: url('backgrounds/blizzard.png');}"""
			rulesUri(css3) must beEqualTo(Seq(".foo .bar" -> "http://akka.io/backgrounds/blizzard.png"))
		}

		"work with a complex CSS background directive" in {
			val css = """body {background:#ffffff url('img_tree.png') no-repeat right top;}"""
			rulesUri(css) must beEqualTo(Seq("body" -> "http://akka.io/img_tree.png"))
		}

		"work with a multiple urls in the CSS" in {
			val css = """body { background-image: url('backgrounds/blizzard.png'); }
				    			.foo { background: url('backgrounds/landscapes/sky.jpg'); }"""
			rulesUri(css) must beEqualTo(Seq("body" -> "http://akka.io/backgrounds/blizzard.png", ".foo" -> "http://akka.io/backgrounds/landscapes/sky.jpg"))
		}
		...
		"work with all URI format" in {
			...
			rulesUri(css) must beEqualTo(Seq("body" -> "http://www.example.com/pinkish.png"))

			// With simple quotes
			val css2 = """li { list-style: url(http://www.example.com/redball.png) disc }"""
			rulesUri(css2) must beEqualTo(Seq("li" -> "http://www.example.com/redball.png"))

			// With nothing
			val css3 = """body { background: url(http://www.example.com/pinkish.png) }"""
			rulesUri(css3) must beEqualTo(Seq("body" -> "http://www.example.com/pinkish.png"))

			// With spaces
			val css4 = """body { background: url( http://www.example.com/pinkish.png ) }"""
			rulesUri(css4) must beEqualTo(Seq("body" -> "http://www.example.com/pinkish.png"))
		}
	}
}
