	...
	def newClient(session: Option[Session]) = {
		...
		val ahcConfig = session.map { session =>
			...
			val trustManagers = for {
				storeType <- session.get[String](CONF_HTTP_SSL_TRUST_STORE_TYPE)
				file <- session.get[String](CONF_HTTP_SSL_TRUST_STORE_FILE)
				password <- session.get[String](CONF_HTTP_SSL_TRUST_STORE_PASSWORD)
				algorithm = session.get[String](CONF_HTTP_SSL_TRUST_STORE_ALGORITHM)
			} yield newTrustManagers(storeType, file, password, algorithm)

			val keyManagers = for {
				storeType <- session.get[String](CONF_HTTP_SSL_KEY_STORE_TYPE)
				file <- session.get[String](CONF_HTTP_SSL_KEY_STORE_FILE)
				password <- session.get[String](CONF_HTTP_SSL_KEY_STORE_PASSWORD)
				algorithm = session.get[String](CONF_HTTP_SSL_KEY_STORE_ALGORITHM)
