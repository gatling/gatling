...
final class WsSetCheck(
    ...
    with ExitableAction
    with NameGen {
  override val name: String = genName("wsSetCheck")
  ...
  override def sendRequest(session: Session): Validation[Unit] =
    for {
      ...
      logger.debug(s"Setting check on WebSocket '$wsName': Scenario '${session.scenario}', UserId #${session.userId}")
      // [e]
      //
      // [e]
