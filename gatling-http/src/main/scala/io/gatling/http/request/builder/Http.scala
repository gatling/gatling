...
class Http(requestName: Expression[String]) {
  ...
  def get(uri: UriComponents) = httpRequest("GET", Right(uri))
  ...
  def httpRequest(method: String, urlOrURI: Either[Expression[String], UriComponents]): HttpRequestBuilder = new HttpRequestBuilder(CommonAttributes(requestName, method, urlOrURI), HttpAttributes())
  ...
  def httpRequestWithParams(method: String, urlOrURI: Either[Expression[String], UriComponents]): HttpRequestWithParamsBuilder = new HttpRequestWithParamsBuilder(CommonAttributes(requestName, method, urlOrURI), HttpAttributes(), Nil)
}
