...
class HttpRequestBaseBuilder(val requestName: String) {
	 ...
	 * @param interpolations session keys for interpolation
	 */
	def post(url: String) = new PostHttpRequestBuilder(requestName, parseEvaluatable(url), Nil, Nil, Map.empty, None, Nil, None, Nil)
	 ...
	 * @param f the function returning the url of this request
	 */
	def post(f: EvaluatableString) = new PostHttpRequestBuilder(requestName, f, Nil, Nil, Map.empty, None, Nil, None, Nil)
