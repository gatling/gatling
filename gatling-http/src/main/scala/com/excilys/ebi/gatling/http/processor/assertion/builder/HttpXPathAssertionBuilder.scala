...
object HttpXPathAssertionBuilder {
  class HttpXPathAssertionBuilder(expression: Option[String], expected: Option[String], attrKey: Option[String], httpPhase: Option[HttpPhase])
    ...
    def build: HttpAssertion =
      if (expected.isEmpty)
        new HttpXPathPresentAssertion(expression.get, attrKey.get, httpPhase.get)
      else
        new HttpXPathAssertion(expression.get, expected.get, attrKey.get, httpPhase.get)
  }
  ...
  def assertXpath(expression: String) = new HttpXPathAssertionBuilder(Some(expression), None, Some(StringUtils.EMPTY), Some(CompletePageReceived))
}