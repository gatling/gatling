...
object HttpRegExpCaptureBuilder {
  class HttpRegExpCaptureBuilder[HE](expression: Option[String], attribute: Option[String], scope: Option[HttpScope], httpHook: Option[HttpResponseHook])
    ...
    def onHeaders = new HttpRegExpCaptureBuilder[HE](expression, attribute, scope, Some(HeadersReceived))
    def onComplete = new HttpRegExpCaptureBuilder[HE](expression, attribute, scope, Some(CompletePageReceived))
  }

  implicit def enableBuild(builder: HttpRegExpCaptureBuilder[TRUE]) = new {
    def build(): HttpCapture = new HttpRegExpCapture(builder.expression.get, builder.attribute.get, builder.scope.get, builder.httpHook.get)
  }

  def regexp(expression: String) = new HttpRegExpCaptureBuilder[TRUE](Some(expression), None, Some(new SessionScope), Some(CompletePageReceived))
