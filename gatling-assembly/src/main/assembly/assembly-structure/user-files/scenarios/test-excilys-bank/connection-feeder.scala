...
val scn = scenario("Test Excilys Bank")
		...
		.doHttpRequest(
		    "Home"
		    , get(urlAccueil)
		    , checkRegexp("""<a href="/excilys-bank-web/logout" class="button blue">Log out</a>""")
		)
		.pause(4)
		.doHttpRequest(
			"Account details"
		    , get(urlFirstAccount, "acc1")
			, checkRegexp("""<table class="accountDetails">""")
		)
		.pause(4)
		.doHttpRequest(
			"Transfers"
		...
		.doHttpRequest(
			...
			, checkRegexp("""<legend>Perform a new transfer</legend>""")
		)
		.pause(4)
		.iterate(
		    20
