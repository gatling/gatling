...
object HarReader {
	...
	def apply(json: Json): List[ScenarioElement] = {
		...
		entries.iterator
			...
			.filter(e => recorderConfiguration.filters.filters.map(_.accept(e.request.url)).getOrElse(true))
	...
	private def createRequest(entry: Entry): RequestElement = {
		...
		val body = entry.request.postData.map { postData =>
			postData.text.trimToOption match {
				case Some(string) => RequestBodyBytes(string.getBytes(recorderConfiguration.core.encoding))
