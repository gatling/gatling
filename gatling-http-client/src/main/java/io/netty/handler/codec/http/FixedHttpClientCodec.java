...
/**
 * A combination of {@link HttpRequestEncoder} and {@link HttpResponseDecoder} which enables easier
 * client side HTTP implementation. {@link HttpClientCodec} provides additional state management for
 * <tt>HEAD</tt> and <tt>CONNECT</tt> requests, which {@link HttpResponseDecoder} lacks. Please
 * refer to {@link HttpResponseDecoder} to learn what additional state management needs to be done
 * for <tt>HEAD</tt> and <tt>CONNECT</tt> and why {@link HttpResponseDecoder} can not handle it by
 * itself.
 *
 * <p>If the {@link Channel} is closed and there are missing responses, a {@link
 * PrematureChannelClosureException} is thrown.
 *
 * @see HttpServerCodec
 */
public final class FixedHttpClientCodec
  ...
  public FixedHttpClientCodec(int maxInitialLineLength, int maxHeaderSize, int maxChunkSize) {
  ...
  public FixedHttpClientCodec(
    ...
    init(
        new Decoder(maxInitialLineLength, maxHeaderSize, maxChunkSize, validateHeaders),
        new Encoder());
  ...
  private final class Encoder extends HttpRequestEncoder {
    ...
    protected void encode(ChannelHandlerContext ctx, Object msg, List<Object> out)
      ...
      if (upgraded) {
        // HttpObjectEncoder overrides .write and does not release msg, so we don't need to retain
        // it here
        out.add(msg);
        return;
      }
  ...
  private final class Decoder extends HttpResponseDecoder {
    ...
    protected boolean isContentAlwaysEmpty(HttpMessage msg) {
      ...
      // If the remote peer did for example send multiple responses for one request (which is not
      // allowed per
