...
class OAuthSignatureCalculatorTest {
  ...
  void testSignatureBaseStringWithProperlyEncodedUri() throws NoSuchAlgorithmException {
    ...
    Request request = new RequestBuilder(HttpMethod.POST, Uri.create("http://example.com/request?b5=%3D%253D&a3=a&c%40=&a2=r%20b"))
      .setBody(new FormUrlEncodedRequestBody(formParams))
      .build(true);
  ...
  void testPostCalculateSignature() throws Exception {
    ...
    final Request req = new RequestBuilder(HttpMethod.POST, Uri.create("http://photos.example.net/photos"))
      .setBody(new FormUrlEncodedRequestBody(postParams))
      .build(true);
    ...
    calc.sign(req.getMethod(), req.getUri(), req.getHeaders(), req.getBody());
  ...
  void testWithNullRequestToken() throws NoSuchAlgorithmException {
    ...
    final Request request = new RequestBuilder(HttpMethod.GET, Uri.create("http://photos.example.net/photos?file=vacation.jpg&size=original")).build(true);
  ...
  void testWithStarQueryParameterValue() throws NoSuchAlgorithmException {
    final Request request = new RequestBuilder(HttpMethod.GET, Uri.create("http://term.ie/oauth/example/request_token.php?testvalue=*")).build(true);
  ...
  void testSignatureGenerationWithAsteriskInPath() throws InvalidKeyException, NoSuchAlgorithmException {
    ...
    final Request request = new RequestBuilder(HttpMethod.GET, Uri.create("http://example.com/oauth/example/*path/wi*th/asterisks*")).build(true);
  ...
  void testPercentEncodeKeyValues() throws Exception {
    ...
    calc.sign(request.getMethod(), request.getUri(), request.getHeaders(), null);
  }
}
