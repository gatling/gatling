...
trait ConditionalStatements[B] extends Execs[B] {
	 ...
	 * @return a new builder with a conditional execution added to its actions
	 */
	def doIf(sessionKey: EvaluatableString, value: String)(thenNext: ChainBuilder): B = doIf(session => sessionKey(session) == value, thenNext, None)
	 ...
	 * @return a new builder with a conditional execution added to its actions
	 */
	def doIfOrElse(condition: Session => Boolean)(thenNext: ChainBuilder)(elseNext: ChainBuilder): B = doIf(condition, thenNext, Some(elseNext))
	 ...
	 */
	def randomSwitch(possibility1: (Int, ChainBuilder), possibilities: (Int, ChainBuilder)*): B = newInstance(RandomSwitchBuilder(possibility1 :: possibilities.toList) :: actionBuilders)
	 ...
	 * @return a new builder with a random switch added to its actions
	 */
	def roundRobinSwitch(possibility1: ChainBuilder, possibilities: ChainBuilder*): B = newInstance(RoundRobinSwitchBuilder(possibility1 :: possibilities.toList) :: actionBuilders)
}