...
import com.excilys.ebi.gatling.core.action.builder.{ ActionBuilder, BypassSessionHookBuilder }
...
trait Execs[B] {
	 ...
	 */
	def exec(sessionFunction: Session => Session): B = exec(new BypassSessionHookBuilder(sessionFunction))
