 ...
 */
class JoddCssExtractor(string: String) extends CssExtractor {

	val nodeSelector = new NodeSelector(new LagartoDOMBuilder().parse(string))

	def extractMultiple(nodeAttribute: Option[String])(expression: String): Option[Seq[String]] = nodeSelector
		.select(expression)
		.map { node =>
