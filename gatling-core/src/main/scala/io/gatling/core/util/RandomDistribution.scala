...
private[core] object RandomDistribution {
  ...
  def percentWeights[T](possibilities: List[(Double, T)], fallback: T): RandomDistribution[T] = {
    ...
    val normalizedTail: List[(Int, T)] = tail.map { case (weight, chain) => (weight * PercentWeightsNormalizingFactor).toInt -> chain } // don't round but truncate so normalized sum doesn't because bigger than original one
