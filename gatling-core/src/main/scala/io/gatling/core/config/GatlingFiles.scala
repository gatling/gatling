...
object GatlingFiles {
  ...
  def binariesDirectory(configuration: GatlingConfiguration) = configuration.core.directory.binaries.map(path => resolvePath(path)).getOrElse(GatlingHome / "target" / "test-classes")
