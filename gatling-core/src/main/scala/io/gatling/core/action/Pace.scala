...
object Pace {
  def props(intervalExpr: Expression[Duration], counter: String, coreComponents: CoreComponents, next: ActorRef) =
    Props(new Pace(intervalExpr, counter, coreComponents.statsEngine, next))
}
