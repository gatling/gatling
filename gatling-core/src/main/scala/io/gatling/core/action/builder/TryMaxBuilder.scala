...
class TryMaxBuilder(times: Int, loopNext: ChainBuilder)(implicit counterName: CounterName) extends ActionBuilder {

	def build(next: ActorRef) = {
		val tryMaxActor = system.actorOf(Props(new TryMax(times, next)))
