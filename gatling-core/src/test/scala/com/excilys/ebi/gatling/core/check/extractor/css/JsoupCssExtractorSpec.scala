...
object JsoupCssExtractorSpec {

	def extractor(file: String) = {
		IOHelper.use(getClass.getResourceAsStream(file)) { is =>
			new JsoupCssExtractor(IOUtils.toString(is, "UTF-8"))
		}
	}
}
...
class JsoupCssExtractorSpec extends Specification {

	import JsoupCssExtractorSpec.extractor
