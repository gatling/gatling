...
class ELSpec extends ValidationSpecification {
  ...
  "One monovalued Expression" should {
    ...
    "properly handle JsonPath expression" in {
      val session = Session("scenario", "1", Map("foo" -> "FOO"))
      val expression = "$.foo.bar".el[String]
      expression(session) must succeedWith("$.foo.bar")
    }

    "properly handle JsonPath expression template" in {
      val session = Session("scenario", "1", Map("foo" -> "FOO"))
      val expression = "$.${foo}.bar".el[String]
      expression(session) must succeedWith("$.FOO.bar")
    }

    "properly handle multiline JSON template" in {
      val session = Session("scenario", "1", Map("foo" -> "FOO"))
      val expression = """{
        "foo": ${foo},
        "bar": 1
      }""".el[String]
      expression(session) must succeedWith("""{
        "foo": FOO,
        "bar": 1
      }""")
    }
