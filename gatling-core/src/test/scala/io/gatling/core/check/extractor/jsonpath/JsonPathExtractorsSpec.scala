...
class JsonPathExtractorsSpec extends ValidationSpecification {
	...
	def prepared(file: String) = {
		...
		withCloseable(getClass.getResourceAsStream(file)) { is =>
			...
			JsonPathExtractors.parse(bytes)
		}
	}
	...
	"extractOne" should {
		...
		"return expected result with last function expression" in {
			JsonPathExtractors.extractOne(0)(prepared("/test.json"), "$.store.book[(@.length - 1)].title") must succeedWith(Some("The Lord of the Rings"))
		}
