...
class GatlingJsonPathExtractorsSpec extends ValidationSpecification {
	...
	"count" should {
		...
		"return expected result with anywhere expression" in {
			GatlingJsonPathExtractors.count(prepared("/test.json"), "$..author") must succeedWith(Some(4))
		}

		"return expected result with array expression" in {
			GatlingJsonPathExtractors.count(prepared("/test.json"), "$.store.book[2].author") must succeedWith(Some(1))
		}
	}
	...
	"extractOne" should {
		...
		"return expected result with anywhere expression and rank 0" in {
			GatlingJsonPathExtractors.extractOne(0)(prepared("/test.json"), "$..author") must succeedWith(Some("Nigel Rees"))
		}

		"return expected result with anywhere expression and rank 1" in {
			GatlingJsonPathExtractors.extractOne(1)(prepared("/test.json"), "$..author") must succeedWith(Some("Evelyn Waugh"))
		}

		"return expected result with array expression" in {
			GatlingJsonPathExtractors.extractOne(0)(prepared("/test.json"), "$.store.book[2].author") must succeedWith(Some("Herman Melville"))
		}
		...
		"not mess up if two nodes with the same name are placed in different locations" in {
			GatlingJsonPathExtractors.extractOne(0)(prepared("/test.json"), "$.foo") must succeedWith(Some("bar"))
		}
		...
		"support element filter with object root" in {
			GatlingJsonPathExtractors.extractOne(0)(prepared("/test.json"), "$..book[?(@.category=='reference')].author") must succeedWith(Some("Nigel Rees"))
		}
	...
	"extractMultiple" should {
		...
		"return expected result with anywhere expression" in {
			GatlingJsonPathExtractors.extractMultiple(prepared("/test.json"), "$..author") must succeedWith(Some(List("Nigel Rees", "Evelyn Waugh", "Herman Melville", "J. R. R. Tolkien")))
		}

		"return expected result with array expression" in {
			GatlingJsonPathExtractors.extractMultiple(prepared("/test.json"), "$.store.book[2].author") must succeedWith(Some(List("Herman Melville")))
		}

		"support wildcard at first level" in {
			GatlingJsonPathExtractors.extractMultiple(prepared("/test2.json"), "$[*].id") must succeedWith(Some(List("19434", "19435")))
		}

		"support wildcard at first level with multiple sublevels" in {
			GatlingJsonPathExtractors.extractMultiple(prepared("/test2.json"), "$..owner.id") must succeedWith(Some(List("18957", "18957")))
		}
