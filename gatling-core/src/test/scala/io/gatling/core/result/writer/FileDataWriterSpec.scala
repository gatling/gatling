...
class FileDataWriterSpec extends BaseSpec {
  ...
  "file data writer" should "log a standard request record" in {
    val record = new ResponseMessage("scenario", "1", Nil, "requestName", ResponseTimings(2L, 3L, 4L, 5L), OK, Some("200"), Some("message"), Nil)
  ...
  it should "append extra info to request records" in {
    ...
    val record = new ResponseMessage("scenario", "1", Nil, "requestName", ResponseTimings(2L, 3L, 4L, 5L), OK, Some("200"), Some("message"), extraInfo)
